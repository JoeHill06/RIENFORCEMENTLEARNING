...
After 30k episodes the combined Q-table holds **28,113** entries — about 14,056 per agent. So around ~2,260 states per agent that averages out to ~6 `(state, action)` entries per state, which is reasonable given that early-game states (few pieces, many empty cells) have up to 9 legal actions each. Coverage isn't literally exhaustive, but it's close to the reachable game tree from each agent's side.
